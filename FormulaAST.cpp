...
virtual double Evaluate(const FormulaAST::CellFunction& cell_func) const = 0;
...
double Evaluate(const FormulaAST::CellFunction& cell_func) const override {
    double lhs = lhs_.get()->Evaluate(cell_func);
    double rhs = rhs_.get()->Evaluate(cell_func);
...
double Evaluate(const FormulaAST::CellFunction& cell_func) const override {
    if (type_ == Type::UnaryMinus) {
        return -operand_.get()->Evaluate(cell_func);
    }
...
double Evaluate(const FormulaAST::CellFunction&) const override {
  return value_;
}
...
double FormulaAST::Execute(const CellFunction& cell_func) const {
